...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        private void t_Tick(object sender, EventArgs e)
        {
            ...
            handCoord = hrCoord(h % 12, m, hr + 4);
            g.DrawLine(new Pen(Color.White, 20f), new Point(cx, cy), new Point(handCoord[0], handCoord[1]));


            //Отрисовка стрелки часов.
            handCoord = hrCoord(h % 12, m, hr);
            g.DrawLine(new Pen(Color.Blue, 4f), new Point(cx, cy), new Point(handCoord[0], handCoord[1]));


            //Отрисовка минутной стрелки
            handCoord = msCoord(m, min);
            g.DrawLine(new Pen(Color.Black, 2f), new Point(cx, cy), new Point(handCoord[0], handCoord[1]));

            // Отрисовка секундной стрелки.
            handCoord = msCoord(s, sec);
            g.DrawLine(new Pen(Color.MediumPurple, 2f), new Point(cx, cy), new Point(handCoord[0], handCoord[1]));
        }
    }
}
